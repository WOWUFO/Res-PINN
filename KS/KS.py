...
# Define MLP
def MLP(layers, L=1.0, M_t=1, M_x=1, activation=relu):
...
def ResNet(layers, L=1.0, M_t=1, M_x=1, activation=relu):
    def input_encoding(t, x):
        w = 2 * np.pi / L
        k_t = np.power(10, np.arange(-M_t // 2, M_t // 2))
        k_x = np.arange(1, M_x + 1)
...
# Define the model
class PINN:
    def __init__(self, key, u_exact, arch, layers, M_t, M_x, state0, t0, t1, n_t, n_x):
        ...
        if arch == 'MLP':
            d0 = 2 * M_x + M_t + 1
            layers = [d0] + layers
            self.init, self.apply = MLP(layers, L=2.0 * np.pi, M_t=self.M_t, M_x=self.M_x, activation=np.tanh)
            params = self.init(rng_key=key)

        if arch == 'ResNet':
            d0 = 2 * M_x + M_t + 1
            layers = [d0] + layers
            self.init, self.apply = ResNet(layers, L=2.0 * np.pi, M_t=self.M_t, M_x=self.M_x, activation=np.tanh)
            params = self.init(rng_key=key)
...
for k in range(N):
    ...
    model = PINN(key, u_exact, arch, layers, M_t, M_x, state0, t0, t1, n_t, n_x)
    ...
    error = np.linalg.norm(u_preds - usol[:, time_step:time_step + (k + 1) * num_step]) / np.linalg.norm(
        usol[:, time_step:time_step + (k + 1) * num_step])
